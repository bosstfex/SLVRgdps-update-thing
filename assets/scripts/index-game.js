...
class us {
  ...
  _spawnLevelObjects(_0x35f1ae) {
    ...
    for (let levelObj of _0x35f1ae) {
      ...
      const _objScale = levelObj.scale || 1;
      ...
      if (frameName) {
        ...
        if (sprite) {
          ...
          if (objectDef && objectDef.type === padType) {
            try {
              sprite._padId = levelObj.id;
              sprite._padWorldX = worldX;
              sprite._padBaseY = baseY;
              sprite._padBaseScale = sprite.scaleX || 1;
              this._padSprites.push(sprite);
            } catch(e) {}
          }
          ...
          if (objectDef && objectDef.type === ringType) {
            sprite.setScale(0.75 * _objScale);
            sprite._eeAudioScale = true;
            this._orbSprites.push(sprite);
            if (_0xOrbGlow) {
              _0xOrbGlow.setScale(0.75 * _objScale);
        ...
        if (objectDef.children) {
          for (let childDef of objectDef.children) {
            let _0x3b4e8c = (childDef.dx || 0) * _objScale;
            let _0x172131 = (childDef.dy || 0) * _objScale;
            if (childDef.localDx !== undefined || childDef.localDy !== undefined) {
              let _0x38902b = (childDef.localDx || 0) * _objScale;
              let _0x256a8e = (childDef.localDy || 0) * _objScale;
              if (levelObj.flipX) {
                _0x38902b = -_0x38902b;
              }
              if (levelObj.flipY) {
            ...
            if (childSprite) {
              ...
              if (childDef.audioScale) {
                childSprite.setScale(0.1 * _objScale);
      ...
      if (objectDef) {
        ...
        if (objectDef.type === solidType && objectDef.gridW > 0 && objectDef.gridH > 0) {
          let _0x10e5ae = objectDef.gridW * a * _objScale;
          let _0x11e08d = objectDef.gridH * a * _objScale;
          ...
          if (objectDef.spriteW > 0 && objectDef.spriteH > 0 && objectDef.hitboxScaleX !== undefined && objectDef.hitboxScaleY !== undefined) {
            _0x3f8c4f = objectDef.spriteW * objectDef.hitboxScaleX * 2 * _objScale;
            _0x2a123d = objectDef.spriteH * objectDef.hitboxScaleY * 2 * _objScale;
          } else if (objectDef.gridW > 0 && objectDef.gridH > 0) {
            _0x3f8c4f = objectDef.gridW * 12 * _objScale;
            _0x2a123d = objectDef.gridH * 24 * _objScale;
          }
          const _hasHitboxRadius = objectDef.hitbox_radius !== undefined && objectDef.hitbox_radius !== null;
          const _worldHitboxRadius = _hasHitboxRadius ? objectDef.hitbox_radius * 2 * _objScale : 0;
          ...
          let _0xad0974 = objectDef.gridW * a * _objScale;
          let _0x2c2226 = objectDef.gridH * a * _objScale;
          ...
          let padW = objectDef.gridW * a * _objScale;
          let padH = objectDef.gridH * a * _objScale;
          ...
          let orbW = objectDef.gridW * a * _objScale;
          let orbH = objectDef.gridH * a * _objScale;
          ...
          let coinW = (objectDef.gridW || 1) * a * _objScale;
          let coinH = (objectDef.gridH || 1) * a * _objScale;
          ...
          let speedW = (objectDef.gridW || 1) * a * _objScale;
          let speedH = (objectDef.gridH || 1) * a * _objScale;
  ...
  updateAudioScale(_0x337bf7) {
    ...
    // pad hit visual scaling (separate base scale)
    for (let _padSpr of (this._padSprites || [])) {
      const _baseScaleP = _padSpr._padBaseScale || 1;
      if (_padSpr._hitTime) {
        const _elapsedP = _now - _padSpr._hitTime;
        if (_elapsedP < 80) {
          const _t = _elapsedP / 80;
          _padSpr.setScale(_baseScaleP + (_baseScaleP * (_clickMult - 1)) * _t);
        } else if (_elapsedP < 80 + _shrinkMs) {
          const _t = (_elapsedP - 80) / _shrinkMs;
          const _peak = _baseScaleP * _clickMult;
          _padSpr.setScale(_peak + (_baseScaleP - _peak) * _t);
        } else {
          _padSpr._hitTime = null;
          _padSpr.setScale(_baseScaleP);
        }
      } else {
        _padSpr.setScale(_baseScaleP);
      }
        ...
        if (_colType === "portal_fly") {
          ...
          if (!gameObj.activated) {
            ...
            if (_padId === 3005) {
              ...
              // Gravpad: instant spiky gravity flip with jolt
              this.flipGravity(!this.p.gravityFlipped, 0);
              this.p.yVelocity = (this.p.gravityFlipped ? -1 : 1) * 28;
              this.p.isJumping = true;
              this.p.onGround = false;
              this.p.canJump = false;
              this.runRotateAction();
...
class xs extends Phaser.Scene {
  ...
  create() {
    ...
    this._openCreatorMenu = () => {
      ...
      this._savedOverlayObjects = [];
      this._loadSavedLevels = () => {
        try {
          return JSON.parse(localStorage.getItem("gd_saved_levels") || "[]") || [];
        } catch (err) {
          console.warn("Failed to read saved levels", err);
          return [];
        }
      };
      this._saveOnlineLevelToStorage = ({ id, name, songKey, artist, levelString, songIdRaw, officialSongId, isCustomSong }) => {
        try {
          const normalizedId = String(id);
          const saved = this._loadSavedLevels();
          const entry = {
            id: normalizedId,
            name: name || `Level ${normalizedId}`,
            songKey: songKey || "",
            artist: artist || "Unknown",
            songIdRaw: songIdRaw || null,
            officialSongId: officialSongId || null,
            isCustomSong: !!isCustomSong,
            savedAt: Date.now()
          };
          const existingIndex = saved.findIndex(item => item.id === normalizedId);
          if (existingIndex >= 0) saved[existingIndex] = entry;
          else saved.unshift(entry);
          localStorage.setItem("gd_saved_levels", JSON.stringify(saved));
          if (levelString) localStorage.setItem("gd_saved_level_" + normalizedId, levelString);
        } catch (err) {
          console.warn("Failed to save online level", err);
        }
      };
      this._getSavedLevelString = (id) => localStorage.getItem("gd_saved_level_" + id);
      this._loadSavedSongForEntry = async (entry) => {
        if (!entry.isCustomSong) return;
        if (window._onlineSongBuffer && window._onlineSongKey === entry.songKey) return;
        const PROXY_BASE = (window._gdProxyUrl || "").replace(/\/$/, "");
        if (!PROXY_BASE) throw new Error("no proxy configured");
        if (!entry.songIdRaw) throw new Error("missing custom song id");

        const ngRes = await fetch(`${PROXY_BASE}/getGJSongInfo.php`, {
          method: "POST",
          headers: { "Content-Type": "application/x-www-form-urlencoded" },
          body: `songID=${entry.songIdRaw}&secret=Wmfd2893gb7`
        });
        if (!ngRes.ok) throw new Error(`song info proxy returned ${ngRes.status}`);
        const ngText = await ngRes.text();
        if (!ngText || ngText === "-1") throw new Error("song info not found");

        const ngParts = ngText.split("~|~");
        const ngMap = {};
        for (let i = 0; i + 1 < ngParts.length; i += 2) ngMap[ngParts[i]] = ngParts[i + 1];
        const rawUrl = (ngMap["10"] || "").trim();
        if (!rawUrl) throw new Error("song URL missing");
        ...
        window._onlineSongKey = entry.songKey;
        window._onlineSongArtist = entry.artist || "Unknown";
      };
      this._playSavedLevel = async (entry, statusText = null) => {
        const setStatus = (msg) => {
          if (statusText) statusText.setText(msg);
        };
        try {
          setStatus("loading saved level...");
          if (entry.isCustomSong) {
            setStatus("loading song...");
            await this._loadSavedSongForEntry(entry);
          }
          const levelString = this._getSavedLevelString(entry.id);
          if (!levelString) throw new Error("saved level data missing");
          window._onlineLevelString = levelString;
          window._onlineLevelId = "online_" + entry.id;
          window.currentlevel = [entry.songKey || "stereo_madness", entry.name, window._onlineLevelId, [entry.artist || "Unknown"]];
          if (entry.isCustomSong) {
            window._onlineSongKey = entry.songKey;
          }
          ...
          console.error("saved level load failed:", err);
      ...
      this._openSavedMenu = () => {
        ...
        const panelWidth = sw - 180;
        const panelHeight = sh - 200;
        const panelX = sw / 2;
        const panelY = sh / 2 + 10;

        const panelFill = this.add.rectangle(panelX, panelY, panelWidth, panelHeight, 0x0b2342, 0.96)
          .setScrollFactor(0).setDepth(102).setOrigin(0.5);
        const panelTop = this.add.image(panelX, panelY - panelHeight / 2 + 16, "GJ_GameSheet03", "GJ_table_top_001.png")
          .setScrollFactor(0).setDepth(103).setDisplaySize(panelWidth, 32).setOrigin(0.5, 0.5);
        const panelBottom = this.add.image(panelX, panelY + panelHeight / 2 - 16, "GJ_GameSheet03", "GJ_table_bottom_001.png")
          .setScrollFactor(0).setDepth(103).setDisplaySize(panelWidth, 32).setOrigin(0.5, 0.5);
        const panelLeft = this.add.image(panelX - panelWidth / 2 + 16, panelY, "GJ_GameSheet03", "GJ_table_side_001.png")
          .setScrollFactor(0).setDepth(103).setDisplaySize(32, panelHeight).setOrigin(0.5, 0.5);
        const panelRight = this.add.image(panelX + panelWidth / 2 - 16, panelY, "GJ_GameSheet03", "GJ_table_side_001.png")
          .setScrollFactor(0).setDepth(103).setDisplaySize(32, panelHeight).setOrigin(0.5, 0.5).setFlipX(true);
        const panelCornerTL = this.add.image(panelX - panelWidth / 2 + 16, panelY - panelHeight / 2 + 16, "GJ_GameSheet03", "GJ_table_corner_001.png")
          .setScrollFactor(0).setDepth(104).setOrigin(0.5);
        const panelCornerTR = this.add.image(panelX + panelWidth / 2 - 16, panelY - panelHeight / 2 + 16, "GJ_GameSheet03", "GJ_table_corner_001.png")
          .setScrollFactor(0).setDepth(104).setOrigin(0.5).setFlipX(true);
        const panelCornerBL = this.add.image(panelX - panelWidth / 2 + 16, panelY + panelHeight / 2 - 16, "GJ_GameSheet03", "GJ_table_corner_001.png")
          .setScrollFactor(0).setDepth(104).setOrigin(0.5).setFlipY(true);
        const panelCornerBR = this.add.image(panelX + panelWidth / 2 - 16, panelY + panelHeight / 2 - 16, "GJ_GameSheet03", "GJ_table_corner_001.png")
          .setScrollFactor(0).setDepth(104).setOrigin(0.5).setFlipX(true).setFlipY(true);

        const backBtn = this.add.image(50, 48, "GJ_GameSheet03", "GJ_arrow_03_001.png")
          .setScrollFactor(0).setDepth(104).setFlipX(true).setFlipY(true)
          .setRotation(Math.PI).setInteractive();
        this._makeBouncyButton(backBtn, 1, () => this._closeSavedMenu());

        const title = this.add.image(sw / 2, 80, "GJ_GameSheet03", "GJ_myLevelsTxt_001.png")
          .setScrollFactor(0).setDepth(105).setOrigin(0.5, 0.5).setScale(0.96);

        const savedLevels = this._loadSavedLevels();
        const listTop = panelY - panelHeight / 2 + 86;
        const rowHeight = 64;
        const rowWidth = panelWidth - 44;
        const savedTextObjects = [];

        const statusText = this.add.bitmapText(panelX, panelY + panelHeight / 2 - 58, "bigFont", "", 18)
          .setScrollFactor(0).setDepth(105).setOrigin(0.5, 0.5).setTint(0xaaddff);
        savedTextObjects.push(statusText);

        if (savedLevels.length === 0) {
          savedTextObjects.push(
            this.add.bitmapText(panelX, panelY, "bigFont", "You have not downloaded any levels yet!", 20)
              .setScrollFactor(0).setDepth(105).setOrigin(0.5, 0.5).setTint(0xffffff),
            this.add.bitmapText(panelX, panelY + 40, "bigFont", "Search a level ID to save it here.", 18)
              .setScrollFactor(0).setDepth(105).setOrigin(0.5, 0.5).setTint(0xaaddff)
          );
        } else {
          const visibleLevels = savedLevels.slice(0, 6);
          visibleLevels.forEach((entry, index) => {
            const y = listTop + index * (rowHeight + 12);
            const rowBg = this.add.image(panelX, y, "GJ_GameSheet03", "GJ_longBtn02_001.png")
              .setScrollFactor(0).setDepth(105).setDisplaySize(rowWidth, rowHeight).setOrigin(0.5).setInteractive();
            const nameLabel = this.add.bitmapText(panelX - rowWidth / 2 + 18, y - 10, "bigFont", entry.name, 18)
              .setScrollFactor(0).setDepth(106).setOrigin(0, 0.5).setTint(0xffffff);
            const idLabel = this.add.bitmapText(panelX - rowWidth / 2 + 18, y + 14, "bigFont", `ID ${entry.id}`, 16)
              .setScrollFactor(0).setDepth(106).setOrigin(0, 0.5).setTint(0xaaddff);
            this._makeBouncyButton(rowBg, 1, async () => {
              await this._playSavedLevel(entry, statusText);
            }, () => true);
            savedTextObjects.push(rowBg, nameLabel, idLabel);
          });
        }

        const footerBtn = this.add.image(panelX, sh - 60, "GJ_GameSheet03", "GJ_longBtn03_001.png")
          .setScrollFactor(0).setDepth(105).setDisplaySize(panelWidth - 40, 60).setOrigin(0.5);
        const footerLabel = this.add.bitmapText(panelX, sh - 60, "bigFont", "ALL", 24)
          .setScrollFactor(0).setDepth(106).setOrigin(0.5, 0.5).setTint(0xffffff);

        this._savedOverlayObjects = [overlay, blocker, panelFill, panelTop, panelBottom, panelLeft, panelRight, panelCornerTL, panelCornerTR, panelCornerBL, panelCornerBR, backBtn, title, footerBtn, footerLabel, ...savedTextObjects];
      };
      this._closeSavedMenu = (silent = false) => {
        if (!this._savedOverlay) return;
        const destroy = () => {
          for (const obj of this._savedOverlayObjects) {
      ...
      menuButtons.forEach((frame, idx) => {
        ...
        if (isSearchButton || isSavedButton || isFeaturedButton) {
          btn.setInteractive();
          this._makeBouncyButton(btn, btnScale, () => {
            if (isSearchButton) {
              ...
              console.log("Featured clicked - feature not implemented yet.");
            }
          }, () => true);
        } else {
    ...
    this._openSearchMenu = () => {
      ...
      const _doSearchInner = async (levelId) => {
        ...
        window._onlineLevelString = levelString;
        this._saveOnlineLevelToStorage({
          id: levelIdParsed,
          name: levelName,
          songKey,
          artist: window._onlineSongArtist || "Unknown",
          levelString,
          songIdRaw,
          officialSongId,
          isCustomSong
